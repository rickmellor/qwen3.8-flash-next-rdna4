...
def profile_cudagraph_memory(runner: "GPUModelRunner") -> int:
    ...
    try:
        with set_current_vllm_config(runner.vllm_config):
            ...
            # runner.block_tables now exists (real, if minimal -- built by
            # the real initialize_kv_cache() call above, is_profiling=True).
            # This is the earliest point in startup a QSA warmup's
            # _dummy_run(force_attention=True) can succeed (see
            # GRAPH-CAPTURE-FIX.md "Gap 3"), and it's also the last point
            # before this same function's runner.capture_model() call below
            # -- the actual first real graph-capture attempt, and the site
            # of the fatal WMMA-during-capture crash on gfx1201. Warming QSA
            # here, in eager mode, right before that capture, is what lets
            # capture_model() below reuse an already-resolved Triton config
            # instead of re-running the (fatal-if-mid-capture) autotuning
            # search itself.
            from vllm.model_executor.warmup.qwen4_exp_amd_qsa_warmup import (
                qwen4_exp_amd_qsa_triton_warmup_runner,
            )

            qwen4_exp_amd_qsa_triton_warmup_runner(runner)
